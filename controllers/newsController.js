...
// /rewrite/news/rss - Legal content from DB
exports.getRewriteRSS = async (req, res) => {
  try {
    const {
      limit = 100, // Default to last 100 news
      language = "mr"
    } = req.query;

    const newsCollection = await getNewsCollection();

    // Fetch latest 100 published news
    const newsItems = await newsCollection
      .find({ status: "published" })
      .sort({ publishedAt: -1 })
      .limit(parseInt(limit))
    ...
    const channelTitle = 'Rewritten News Feed';
    const channelDescription = 'AI-rewritten legal news feed';
    const channelLink = `${baseUrl}/news`;
...
`;
    ...
    for (const item of newsItems) {
      const title = (item.title || '').trim();
      // Link to our own detail page or original? User said "legal content", implying we own it now? 
      // But typically Inshorts model links back to original. 
      // However, for RSS validity, we can use our own link logic.
      const link = item.link || `${baseUrl}/news/${item._id}`;
      let guid = link;
      if (!guid || !isValidUrl(guid)) guid = `${baseUrl}/news/${item._id}`;

      const isGuidUrl = isValidUrl(guid);

      // Use rewritten content
      let description = item.rewrittenContent || item.originalSummary || '';
      if (description.includes('<')) description = cleanDescription(description);

      const pubDate = item.publishedAt ? formatRFC822Date(new Date(item.publishedAt)) : formatRFC822Date(new Date());
...
`;
    ...
    console.error("Error generating Rewrite RSS:", error);
    res.status(500).send("Error generating RSS feed");
  }
};
