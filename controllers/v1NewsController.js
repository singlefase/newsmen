    ...
    for (const src of sourcesToFetch) {
      ...
      try {
        ...
        for (const item of feed.items) {
          ...
          if (!r2ImageUrl) {
            const stockImg = await getStockImage(detectedCategories);
            if (stockImg?.url) {
              const stockResult = await downloadAndUploadImage(
                stockImg.url,
                `stock-${stockImg.source}`
              );
              if (stockResult.success) {
                r2ImageUrl = stockResult.url;
                imageDownloaded = true;
                imageUploaded = true;
                stockImageSource = stockImg.source;
                console.log(`  ✅ Stock image (${stockImg.source}) uploaded: ${stockResult.url.substring(0, 70)}...`);
              }
            }
          }
